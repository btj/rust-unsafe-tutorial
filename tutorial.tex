...
sometimes incorrectly report ``0 errors found''; see \url{https://verifast.github.io/verifast/rust-reference/\#soundness-of-verifast}. There
may also be unknown unsoundnesses.}, among other things, that the program
\begin{itemize}
...
We will now proceed to introduce the tool's features for verifying purely \lstinline|unsafe| programs step by
...
Throughout this tutorial, you will find exercises that ask you to annotate a small Rust program so that it is accepted by VeriFast. You can find solutions to all exercises at the end of this tutorial. Furthermore, un-annotated and annotated versions of each solution are included in the VeriFast distribution in directories \verb|rust_tutorials/purely_unsafe/problems| and \verb|rust_tutorials/purely_unsafe/solutions|, respectively.

Note: In this tutorial, we do not address the following complexities of verifying Rust programs:
\begin{itemize}
\item We do not verify that the cleanup performed during stack unwinding after a panic is safe, because VeriFast does not yet have good support for doing so.\footnote{\url{https://github.com/verifast/verifast/issues/859}} Therefore, we run VeriFast with the \verb|-ignore_unwind_paths| flag. This is sound only when compiling with \verb|-C panic=abort|.
\item We avoid creating shared references, mutable references, or boxes, so that we can ignore Rust's pointer aliasing rules and rules on mutating immutable bytes\footnote{\url{https://doc.rust-lang.org/reference/behavior-considered-undefined.html}}. So far, VeriFast has only partial support for verifying compliance with these rules.\footnote{\url{https://github.com/verifast/verifast/issues/685}}
\item We avoid verifying non-\lstinline|unsafe| functions, which involves verifying that they are \emph{sound}\footnote{\url{https://doc.rust-lang.org/nomicon/working-with-unsafe.html}}, i.e.~safe for use by code verified only by the Rust type checker. Resources on this topic are available elsewhere\footnote{\url{https://verifast.github.io/verifast/rust-reference/non-unsafe-funcs.html}}.
\end{itemize}
...
\ref{section:memcpy} offer an alternative introduction to
...
nonnegative integers, for each of these integers $N$ computes the $N$'th Fibonacci number (modulo $2^{64}$), sums
up the results (again modulo $2^{64}$), and prints the sum to the console.
\begin{lstlisting}
...
First of all, we need to update the Rust program so that it
properly deallocates all dynamically allocated memory. We need to introduce a function \lstinline|Tree::dispose| and update
function \lstinline|main|:
\begin{lstlisting}
impl Tree {
...
Updating the annotations is slightly trickier. Consider the  \lstinline|Tree::dispose|
call in function \lstinline|main|. It needs full permission for
the tree, i.e.~it needs a \lstinline|Tree(tree, _)| chunk.
Notice that each \lstinline|join| call yields a
\lstinline|[1/2]Tree(tree, _)| chunk. We have two half chunks, and we
...
pred bar(x: *mut i32, y: *mut i32) = foo(?b) &*& if b { *x |-> 0 } else { *y |-> 0 };
...
\end{lstlisting}
At the end of the \lstinline|main| function, if \lstinline|b| is true,
we have a fraction 1.5 of the points-to chunk for \lstinline|x|, and
otherwise we have a fraction 1.5 of the points-to chunk for \lstinline|y|.
...
Let's verify a program that reads a fixed number of bytes
from standard input and then writes them out twice.
Here's a version that reads 5 bytes:
\begin{lstlisting}
...
However, clearly this approach is not practical for large numbers of bytes. One approach that works for large numbers of bytes is to use an array of bytes. We allocate
the array using an \lstinline!alloc! function and then read and write the bytes using a recursive function. For now, we don't worry about freeing the array at the end
of the program. Let's first again do a version that reads 5 bytes.
\begin{lstlisting}
use std::alloc::{Layout, alloc, handle_alloc_error};
unsafe fn read_byte() -> u8 { ... }
unsafe fn write_byte(value: u8) { ... }
...
So, let's solve Exercise~\ref{exercise:bytes} by using \lstinline!bytes_! and \lstinline|bytes| in the contracts:
\begin{lstlisting}
...
\end{lstlisting}
This doesn't quite work yet. The problem is that VeriFast does not automatically replace a \lstinline!bytes_! or \lstinline|bytes| chunk with its definition, or vice versa. You need to do this explicitly by inserting
...
\lstinline!read_bytes! because it cannot find a chunk matching \lstinline!points_to_(start, _)!. This chunk is in fact present; it's just that it is hidden inside the \lstinline!bytes_! chunk. We need to
open up the \lstinline!bytes_! chunk so that VeriFast can see the \lstinline!points_to_! chunk that is inside of it.
The following version of function \lstinline!read_bytes! verifies:
\begin{lstlisting}
unsafe fn read_bytes(start: *mut u8, count: usize)
//@ req bytes_(start, count);
//@ ens bytes(start, count);
...
\end{lstlisting}
Inserting analogous commands in function \lstinline!write_bytes! yields a correct solution of Exercise~\ref{exercise:bytes}.
...
\subsection{Exercise \ref{exercise:bytes}: bytes.rs}\label{solution:characters}
...
\subsection{Exercise \ref{exercise:xor}: xor.rs}\label{solution:xor}
...
\subsection{Exercise \ref{exercise:bytes_loop}: bytes\_loop.c}\label{solution:bytes_loop}
...
\subsection{Exercise \ref{exercise:tuerk}: tuerk.rs}\label{solution:tuerk}
...
\subsection{Exercise \ref{exercise:stack_tuerk}: stack\_tuerk.rs}\label{solution:stack_tuerk}
